...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
    ...
    protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
        ...
        try {
            mbdToUse.prepareMethodOverrides();//处理ovverride属性
    ...
    protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
        ...
        // Instantiate the bean.    BeanWrapper ，包含了真正的bean对象和bean的class，以及PropertyDescriptor集合
        BeanWrapper instanceWrapper = null;
        if (mbd.isSingleton()) {//单例的情况下尝试从factoryBeanInstanceCache获取 instanceWrapper,并清除缓存
            instanceWrapper = this.factoryBeanInstanceCache.remove(beanName);
        }
        if (instanceWrapper == null) {//如果没有则需要自己创建
            instanceWrapper = createBeanInstance(beanName, mbd, args);
        }
        final Object bean = instanceWrapper.getWrappedInstance();
        Class<?> beanType = instanceWrapper.getWrappedClass();
        if (beanType != NullBean.class) {//如果不是NullBean，则将resolvedTargetType 属性设置为当前的WrappedClass
        ...
        //	如果earlySingletonExposure，尝试从缓存获取该bean（一般存放在singletonFactories对象通过调用getObject
        // 	把对象存入earlySingletonObjects），分别从singletonObjects和earlySingletonObjects获取对象
        if (earlySingletonExposure) {
        ...
        try {
            registerDisposableBeanIfNecessary(beanName, bean, mbd); //如果配置了destroy-method,需要注册以便于销毁时调用
        } catch (BeanDefinitionValidationException ex) {
            throw new BeanCreationException(
